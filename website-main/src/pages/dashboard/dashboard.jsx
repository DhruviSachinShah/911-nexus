...
function Dashboard() {
  ...
  const [medicalCalls, setMedicalCalls] = useState(0);
  const [accidentCalls, setAccidentCalls] = useState(0);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "newCall") {
        ...
        } else if (data.callType === "Medical") {
          setMedicalCalls((prevCount) => prevCount + 1);
        } else if (data.callType === "Accident") {
          setAccidentCalls((prevCount) => prevCount + 1);
        }
      }
